...
def find_free_port(preferred_port=DEFAULT_PORT, host=DEFAULT_HOST):
...
def get_port_from_env():
    raw_port = os.environ.get("PDF_TOOL_PORT") or os.environ.get("PORT")
    if not raw_port:
        return DEFAULT_PORT
    try:
        port = int(raw_port)
        ...
        pass
    print(f"端口设置无效，已改用默认端口 {DEFAULT_PORT}: {raw_port}")
    return DEFAULT_PORT
...
def download_file(filename):
    ...
    if os.path.commonpath([str(safe_dir), str(target)]) != str(safe_dir):
        return "禁止访问", 403
...
if __name__ == '__main__':
    host = os.environ.get("PDF_TOOL_HOST", DEFAULT_HOST)
    preferred_port = get_port_from_env()
    port = find_free_port(preferred_port, host)
    url = f"http://{host}:{port}"

    print("=" * 50)
    print("PDF 工具集已启动")
    if port != preferred_port:
        print(f"端口 {preferred_port} 已被占用，已自动切换到端口 {port}")
